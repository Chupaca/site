...
function saveNewPage() {
    ...
    let sitemap = {};
    sitemap.MetaData = GetMetaData();
    sitemap.Header = GetHeaderData();
    sitemap.Language = $("#language_select option:selected").val() || 0;
    if (sitemap.MetaData && sitemap.Header && sitemap.Language) {
        ConformModal("האם אתה רוצה לשמור גרסה חדשה?", () => {
            SaveNewPageToServer(sitemap, prefixLanguage + "sitemap");
            return;
        })
    } else {
        if(!sitemap.Language){
